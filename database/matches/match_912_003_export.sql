 ...
 * from cd_012_03
where length(trav_name)>1 and dep_date!='[no data]' 
and length(dep_time)>1
)
...
(
select 
to_timestamp(dep_date||' '||dep_time,'DD.MM.YYYY HH24:MI')::timestamp without time zone as dep_sched_local ,
* from prep1
where sep_pos>0),
...
replace(replace(replace(replace(fist_name, 'Mrs', ''), 'mrs', ''), 'Mr', ''), 'mr', '') as first_name_c, * from prep3
),
...
(
	...
	--c.airline_iata as airline_iata,
	c.flightnumber_op as flightnumber,
	date_trunc('minute',dep_sched_local) as dep_sched_local_min,
	date_trunc('hour',dep_sched_local) as dep_sched_local_hour,
	*	
	from  prep4 c
...
'EUR' as Währung, 
ticketbruttobetrag_eur as Tick_Preis,
'' as Tick_Geb, 
'' as Tick_ST_7,
'' as Tick_ST_19, 
