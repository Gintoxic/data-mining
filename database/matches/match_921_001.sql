...
prep2 as
(select import_counter, origin_iata, dest_iata, airline_iata, flightnumber, 
to_timestamp(dep_sched_date||' '||dep_sched_time,'DD.MM.YYYY HH24:MI')::timestamp without time zone as dep_sched_local 
from prep1
),
--select * from prep2
...
select case when cancelled='Cancelled' then 'Cancelled' else 'Delayed' end as status,  dist,
count(*), count(distinct flight_id), 
count(distinct import_counter), 
sum(airline_opva)
 from match
group by cancelled, dist
