...
(
	...
	flight_id, cancelled, diftime_utc, type_dist, airline_vec from v_flights
),
prep1 as
(
select *
...
(
	...
	vr.* ,f.dep_sched_local as matched_dep_sched_local, f.flight_id, f.cancelled, f.diftime_utc, f.airline_opva, f.airline_vec,
...
select airline_vec, case when cancelled='Cancelled' then 'Cancelled' else 'Delayed' end as status, count(*), count(distinct flight_id), count(distinct import_counter) from match
group by cancelled, airline_vec
order by cancelled, airline_vec
