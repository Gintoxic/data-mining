...
date_trunc('hour', dep_sched_local) as dep_sched_local_hour, 
	...
	flight_id, cancelled, diftime_utc, type_dist, airline_opva from v_flights
),
...
match as
(
	select vr.*, f.flight_id, f.cancelled, f.diftime_utc, f.type_dist, f.airline_iata, f.airline_opva
	--f.*
...
(
	...
	match_al as 
(
	select * from match m inner join airlines al
	on m.airline_iata = al.iata
)
select * from match_al
where origin_iata in ('BRN', 'ZRH') or dest_iata in ('BRN', 'ZRH')


/*
select cancelled, airline_opva, --airline_iata, name_airline,
--case when type_dist =1 then 'kruz'
--when type_dist=2 then 'mittel'
--when type_dist=3 then 'lang' end as dist
count(*), count(distinct flight_id) from match_al
--where origin_iata in ('BRN', 'ZRH') or dest_iata in ('BRN', 'ZRH')
group by cancelled,airline_opva --, airline_iata, name_airline
order by cancelled,airline_opva, count(*) desc
*/